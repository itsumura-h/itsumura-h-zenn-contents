...
## ライフタイムを追跡するhooks
Nimの標準的な文字列型やseq型、その他の標準的なコレクションのメモリ管理は、いわゆる「Lifetime-tracking hooks」、つまり特定の型拘束演算子によって行われます。
各オブジェクト型T（Tは個別型でもよい）に対して3つの異なるフックがあり、 コンパイラによって暗黙のうちに呼び出されます。
(注意: ここでの「フック」という言葉は、動的結合や実行時の間接参照 を意味するものではなく、暗黙の呼び出しは静的に結合され、インライン化される可能性 もある)。
