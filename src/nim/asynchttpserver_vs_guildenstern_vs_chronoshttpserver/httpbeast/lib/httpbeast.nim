import
  ...
  std/logging,
  std/macros,
  std/times # TODO this shouldn't be required. Nim bug?
...
#                   data.reqFut = onRequest(request, rdb)
...
# proc eventLoop(
#   params: tuple[onRequest: OnRequest, settings: Settings, isMainThread: bool, rdb:Rdb]
# ) =
#   let (onRequest, settings, isMainThread, rdb) = params
...
#     processEvents(selector, events, ret, onRequest, rdb)
...
# proc run*(onRequest: OnRequest, settings: Settings, initDb:proc():Rdb) =
...
#   if numThreads > 1:
#     when compileOption("threads"):
#       # var threads = newSeq[Thread[(OnRequest, Settings, bool, Rdb)]](numThreads - 1)
#       # for t in threads.mitems():
#       #   createThread[(OnRequest, Settings, bool, Rdb)](
#       #     t, eventLoop, (onRequest, settings, false, initDb())
#       #   )
#       var threads = newSeq[Thread[(OnRequest, Settings, bool, Rdb)]](numThreads)
#       var params:seq[(OnRequest, Settings, bool, Rdb)]
#       for i in 1..numThreads:
#         params.add((onRequest, settings, false, initDb()))
...
#   eventLoop((onRequest, settings, true, initDb()))

# proc run*(onRequest: OnRequest, initDb:proc():Rdb) {.inline.} =
#   ## Starts the HTTP server with default settings. Calls `onRequest` for each
#   ## request.
#   ##
#   ## See the other ``run`` proc for more info.
#   run(onRequest, initSettings(port=Port(8080), bindAddr="", domain=Domain.AF_INET), initDb)
...
template serveWithPlugin*(onRequest, routes, createPlugin:untyped):untyped =
  type OnRequest* = proc (req: Request, routes:Routes, createPlugin:proc():Plugin): Future[void] {.gcsafe.}

  proc processEvents(selector: Selector[Data],
                    events: array[64, ReadyKey], count: int,
                    onRequest: OnRequest,
                    routes:Routes,
                    createPlugin:proc():Plugin) =
    for i in 0 ..< count:
      let fd = events[i].fd
      var data: ptr Data = addr(selector.getData(fd))
      # Handle error events first.
            ...
            if data.data.len >= 4 and fastHeadersCheck(data) or slowHeadersCheck(data):
              ...
              if likely(not waitingForBody):
                for start in parseRequests(data.data):
                  ...
                  if validateRequest(request):
                    data.reqFut = onRequest(request, routes, createPlugin)
                    if not data.reqFut.isNil:
                      capture data:
                        data.reqFut.addCallback(
                          proc (fut: Future[void]) =
  ...
  proc eventLoop(
    params: tuple[onRequest: OnRequest, settings: Settings, isMainThread: bool, routes:Routes, createPlugin:proc():Plugin]
  ) =
    let (onRequest, settings, isMainThread, routes, createPlugin) = params
    ...
    while true:
      let ret = selector.selectInto(-1, events)
      processEvents(selector, events, ret, onRequest, routes, `createPlugin`)
  ...
  run(onRequest, `routes`, `createPlugin`)
